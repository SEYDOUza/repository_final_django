...
class ProjetForm(forms.ModelForm):
    class Meta:
        model = Projet
        fields = ['matiere', 'intituleProjet',
                  'fichierProjet', 'description', 'deadline']
...
class SignUpFormEnseignant(UserCreationForm):
    ...
    is_enseignat = forms.BooleanField(
        initial=True, required=True, widget=forms.HiddenInput)

    class Meta:
        model = User
        fields = ('username', 'email', 'password1', 'password2',
                  'is_enseignat', 'numero', 'matiere_enseignant', 'birth_date', 'first_name', 'last_name')
...
class SignUpFormEtudiant(UserCreationForm):
    ...
    is_etudiant = forms.BooleanField(
        initial=True, required=True, widget=forms.HiddenInput)

    class Meta:
        model = User
        fields = ('username', 'email', 'password1', 'password2',
                  'is_etudiant', 'numero', 'filire_etudiant', 'birth_date', 'first_name', 'last_name')
...
class SignUpFormAdimin(UserCreationForm):
    ...
    is_administrateur = forms.BooleanField(
        initial=True, required=True, widget=forms.HiddenInput)

    class Meta:
        model = User
        fields = ('username', 'email', 'password1', 'password2',
                  'is_administrateur', 'numero', 'birth_date', 'first_name', 'last_name')
